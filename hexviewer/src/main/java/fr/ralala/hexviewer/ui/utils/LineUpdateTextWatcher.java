...
public class LineUpdateTextWatcher implements TextWatcher {
    ...
    private void processNewStringEmpty() {
        mResult.setTextColor(ContextCompat.getColor(mContext, R.color.colorResultWarningHex));
    ...
    private void processResultAndError(String strNew) {
        ...
        if (!SysHelper.isEven(validate.length()) && validate.matches("\\p{XDigit}+"))
            ...
            isError = true;
            mResult.setTextColor(ContextCompat.getColor(mContext, R.color.colorResultErrorHex));
        } else
            mResult.setTextColor(ContextCompat.getColor(mContext, R.color.colorResultSuccessHex));

        mResult.setText(SysHelper.hex2bin(validate));
        if (!validated) {
            mLayout.setErrorTextAppearance(isError ? R.style.AppThemeHex_ErrorTextAppearance : R.style.AppThemeHex_WarningTextAppearance);
